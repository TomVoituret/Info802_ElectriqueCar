...
const MyMap = ({ routeGeometry }) => {
  ...
  useEffect(() => {
    ...
    if (mapRef.current && routeGeometry && routeGeometry.features.length > 0) {
      ...
      if (coordinates.length > 0) {
        ...
        const center = [coordinates[middleIndex][1], coordinates[middleIndex][0]];
  ...
  return (
    <MapContainer ref={mapRef} center={[51.505, -0.09]} zoom={13} style={{ height: "600px", width: "100%", paddingBottom: "5%" }}>
      <TileLayer
      ...
      {routeGeometry && (
        <React.Fragment>
          <Polyline positions={routeGeometry.features[0].geometry.coordinates.map(coord => [coord[1], coord[0]])} color="blue" />
